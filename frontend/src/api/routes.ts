...
export async function createIntakeLink(agencyId: string, opts?: IntakeLinkOpts): Promise<IntakeLink> {
  const { data } = await client.post('/intake/intake-link', { agencyId, ...(opts || {}) })
  return data
}
...
export async function submitIntake(token: string, draft: IntakeDraft): Promise<IntakeResult> {
  const { data } = await client.post('/intake/intake', { token, ...draft })
  return data
}

// 已生成提交链接列表（PandaKing 控制台「复制历史」管理）
export async function listIntakeLinks(): Promise<IntakeLinkView[]> {
  const { data } = await client.get('/intake/intake-links')
  return data
}
...
export async function copyIntakeLink(token: string): Promise<{ copies: number; lastCopiedAt: string | null }> {
  const { data } = await client.post(`/intake/intake-link/${token}/copy`)
  return data
}
...
export async function deleteIntakeLink(token: string): Promise<{ ok: boolean }> {
  const { data } = await client.delete(`/intake/intake-link/${token}`)
  return data
}
